...
# CONFIG — PRÉ-TREINO
# =========================================================
img_size         = 256   # resolução nativa do DINOv3 (não 224)
...
save_path = "/media/gledson/Dados1/Classificação_GastroIntestinal/classificar_original/dinov3_pretrain/"
os.makedirs(save_path, exist_ok=True)
best_weights_path = os.path.join(save_path, "best_weights.pth")

# =========================================================
# MODELO BACKBONE — usado para extrair transforms corretos
# Nome completo com prefixo hf_hub:timm/ obrigatório
# =========================================================
MODEL_NAME = "hf_hub:timm/vit_huge_plus_patch16_dinov3.lvd1689m" # "hf_hub:timm/vit_small_patch16_dinov3.lvd1689m"

# Cria backbone temporário só para extrair data_config
_tmp_model  = timm.create_model(MODEL_NAME, pretrained=False, num_classes=0)
data_config = timm.data.resolve_model_data_config(_tmp_model)
del _tmp_model

# Transforms gerados pelo timm — normalização e resize corretos para o modelo
train_tfms_timm = timm.data.create_transform(**data_config, is_training=True)
val_tfms_timm   = timm.data.create_transform(**data_config, is_training=False)

# Augmentations gastrointestinais adicionais aplicadas ANTES do pipeline do timm
extra_train_aug = transforms.Compose([
...
# =========================================================
# DATASET CUSTOMIZADO
# Aplica augmentation gastrointestinal + pipeline timm
# =========================================================
class GastroDataset(Dataset):
    def __init__(self, folder, extra_aug=None, timm_tfms=None):
        self.ds         = datasets.ImageFolder(folder)
        self.extra_aug  = extra_aug
        self.timm_tfms  = timm_tfms
...
train_ds = GastroDataset(train_dir, extra_aug=extra_train_aug, timm_tfms=train_tfms_timm)
val_ds   = GastroDataset(valid_dir, extra_aug=None,            timm_tfms=val_tfms_timm)
test_ds  = GastroDataset(test_dir,  extra_aug=None,            timm_tfms=val_tfms_timm)
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x):
        # x: (B, N, dim) — sequência de tokens de patch
        w = self.attn(x)
        w = torch.softmax(w, dim=1)
        return torch.sum(w * x, dim=1)   # (B, dim)


# =========================================================
# MODEL — DINOv3 ViT-Small/16 via timm
#
# vit_small_patch16_dinov3.lvd1689m:
#   - 12 blocos (blocks.0 .. blocks.11)
#   - hidden_size = 384
#   - 21.6M parâmetros
#   - Resolução nativa: 256×256
#   - Distilado do DINOv3 ViT-7B no dataset LVD-1689M
#
# forward_features retorna (B, 1+N_patches, 384)
#   índice 0     → cls token (descartado)
#   índices 1..N → tokens de patch (usados no AttentionPooling)
# =========================================================
class DinoV3Classifier(nn.Module):
    def __init__(self):
        ...
        self.backbone = timm.create_model(
            MODEL_NAME,
            pretrained=True,
            num_classes=0   # remove cabeça classificadora original
        )
        dim = self.backbone.num_features   # 384 para ViT-Small
    ...
    def forward(self, x):
        x = self.backbone.forward_features(x)  # (B, 1+N, 384)

        if isinstance(x, tuple):
            x = x[0]

        if x.ndim == 3:
            # Descarta cls token (índice 0) — mantém apenas tokens de patch
            x = x[:, 1:, :]   # (B, N, 384)
            x = self.pool(x)   # (B, 384)
...
# =========================================================
# MÉTRICAS — definida uma única vez
# =========================================================
...
# =========================================================
# LOOP DE TREINAMENTO — PRÉ-TREINO
# =========================================================
print('\n================================================')
print('PRÉ-TREINO — DINOv3 ViT-Small (backbone congelado)')
print('================================================\n')
...
# =========================================================
print('\n================================================')
print('AVALIAÇÃO NO TESTE — Pré-treino')
print('================================================\n')
...
###############################################################################
print("###############################################################################")
print("Fine-Tuning - DINOv3 ViT-Small")
print("###############################################################################")
###############################################################################
...
fine_tune_at      = 4   # fine_tune_at      = 6, blocos 0..5 congelados, 6..11 treináveis
                          # DINOv3 ViT-Small tem 12 blocos (blocks.0..blocks.11)
lr_finetuning     = 1e-5
...
# PATHS — FINE-TUNING
# =========================================================
save_path_pretrain   = save_path
save_path_finetuning = "/media/gledson/Dados1/Classificação_GastroIntestinal/classificar_original/dinov3_finetuning/"
os.makedirs(save_path_finetuning, exist_ok=True)

best_weights_pretrain   = os.path.join(save_path_pretrain,   "best_weights.pth")
...
#
# DINOv3 ViT-Small no timm:
#   backbone.patch_embed  → embedding de patches
#   backbone.blocks.0     → bloco 0
#   ...
#   backbone.blocks.11    → bloco 11
#   backbone.norm         → LayerNorm final
#
# Com fine_tune_at=6: congela blocos 0-5, treina blocos 6-11 + norm
# =========================================================
print(f'\nDescongelando blocos >= {fine_tune_at} do backbone DINOv3...')
...
# =========================================================
# CURVAS — FINE-TUNING (figuras separadas)
# =========================================================
plt.figure(figsize=(10, 5))
plt.plot(train_losses, label='Train Loss')
plt.plot(val_losses,   label='Val Loss')
plt.title('Loss — Fine-tuning DINOv3 ViT-Small')
plt.xlabel('Época')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
plt.plot(val_praucs,   label='Val PR-AUC')
plt.plot(val_aucs,     label='Val ROC-AUC')
plt.title('AUC — Fine-tuning DINOv3 ViT-Small')
plt.xlabel('Época')
plt.ylabel('AUC')
plt.legend()
plt.grid(True)
...
# =========================================================
print('\n================================================')
print('AVALIAÇÃO NO TESTE — Fine-tuning')
print('================================================\n')
...
# Salvar .mat
sio.savemat(save_path_finetuning + 'test_labels.mat',
            {'test_labels': y_true_test})
sio.savemat(save_path_finetuning + 'predictions_test.mat',
            {'predictions_test': y_prob_all})
sio.savemat(save_path_finetuning + 'test_predict.mat',
