...
save_path_fusion = os.path.join(save_path, "fusion_model_eff_conv/")
...
class DatasetTriploGastro(Dataset):
    def __init__(self, dir_caminho, transform_effic, transform_conv):
    ...
    def __getitem__(self, idx):
        ...
        return img_effic, img_conv, label

# Instanciando DataLoaders Unificados
train_ds = DatasetTriploGastro(train_dir, train_tfms_effic, train_tfms_conv)
val_ds   = DatasetTriploGastro(valid_dir, val_tfms_effic, val_tfms_conv)
test_ds  = DatasetTriploGastro(test_dir, test_tfms_effic, test_tfms_conv)
...
class LateFusionClassifier(nn.Module):
    def __init__(self, model_effic, model_conv, num_classes=2):
        super().__init__()
        ...
        # 256 (EfficientNetV2-M) + 256 (ConvNeXtV2)
    ...
    def forward(self, x_effic, x_conv, ):  # três entradas
        with torch.no_grad():
        ...
        x_combinado = torch.cat((out_effic, out_conv), dim=1)  # (B, 768)
        return self.novo_head(x_combinado)

modelo_fusao = LateFusionClassifier(model_effic, model_conv, num_classes=num_classes).to(device)
...
print('\n================================================')
print('LATE FUSION TRAINING (EfficientNetV2M + ConvNeXtV2)')
print('================================================\n')
...
for epoch in range(epochs_finetuning):
    ...
    for x_effic, x_conv, y in train_loader:   # 4 elementos
        ...
        with torch.amp.autocast("cuda"):
            logits = modelo_fusao(x_effic, x_conv)
    ...
    with torch.no_grad():
        for x_effic, x_conv, y in val_loader:   # 4 elementos
            x_effic = x_effic.to(device)
            x_conv  = x_conv.to(device)
            y       = y.to(device)
...
with torch.no_grad():
    for x_effic, x_conv, y in test_loader:   # 4 elementos
        x_effic = x_effic.to(device)
        x_conv  = x_conv.to(device)
        y       = y.to(device)

        with torch.amp.autocast("cuda"):
            logits = modelo_test_fusion(x_effic, x_conv, )
